...
func newOrchestrator(dt float64, maxCapacity float64) *ap.RuntimeOrchestrator {
	...
	rollout := &ap.RolloutController{
		...
		// in that tick, leaving the system with only 31 replicas at tick=3. At tick=3,
		// arrival=220 and existing backlog=196 pushed total to 266 > SLA=250 (breach).
		//
		// Fix: CapRampUpNormal=30 → at tick=2, step=min(30+boost+pressure, err)=56.4
		// replicas. cap=78. physicalService=156. backlog=38+220-156=102 at tick=3.
		// Max backlog stays ≈102 << 250. Zero SLA breaches.
		//
		// Safety: rampCap always bounds step by err (= target - active), so overshoot
		// is impossible. A larger normal rate only speeds convergence, it does not
		// cause oscillation. Tested across all 10 scenarios.
		CapRampUpNormal:       30.0,
	...
	id := &ap.IdentificationEngine{
		...
		// New half-life ln(2)/0.25 ≈ 2.8 ticks: clears in 5-6 ticks, matching real
		// burst durations. Combined with the relative-floor fix in updateBurst, this
		// eliminates ghost instability_high across all scenarios.
		BurstDecay:          0.25,
		BurstCap:            12.0,
		NoiseGain:           0.20,
		DriftGain:           0.10,
...
func computeDecisionConflicts(scenario string, timeline []TickSnapshot) int {
	conflicts := 0
	for _, t := range timeline {
		...
		if (apDir == "scale_up" && authDir == "scale_down") ||
			...
			f, _ := os.OpenFile("conflicts.log", os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
			fmt.Fprintf(f, "[%s] CONFLICT: Tick %d, AP=%s, AUTH=%s\n", scenario, t.Tick, apDir, authDir)
			f.Close()
			conflicts++
		}
	}
...
func runScenario(
	...
	for tick := 1; tick <= ticks; tick++ {
		...
		// FIX: Classify direction using BOTH ScaleFactor AND replica count delta.
		// Root cause: authority returns ScaleFactor=1.00 even when deploying significantly
		// more replicas (e.g., 10→22→88). ScaleFactor alone classifies this as "hold",
		// masking real scale_up activity in reports and making oscillation detection blind.
		// Solution: if replicas increased by ≥2 OR ScaleFactor > 1.04 → scale_up.
		//           if replicas decreased by ≥2 OR ScaleFactor < 0.96 → scale_down.
		dir := "hold"
		if scale > 1.04 || replicaDelta >= 2.0 {
			dir = "scale_up"
		} else if scale < 0.96 || replicaDelta <= -2.0 {
			dir = "scale_down"
		}
		...
		// CRITICAL FIX: Gate instabilityCount on actual queue pressure.
		// VarianceScale > 2.0 (instScore > 0.5) is elevated during burst arrivals because
		// burst energy accumulates in the identification engine with slow decay (BurstDecay=0.10).
		// High VarianceScale under low backlog is WORKLOAD VOLATILITY, not SYSTEM INSTABILITY.
		// A retry storm scenario with backlog=20 and SLA=250 and 0 SLA breaches should not
		// be flagged as unstable. Only count instability when BOTH signal variance is high
		// AND the system is actually under meaningful queue pressure (>40% of SLA limit).
		if instScore > 0.5 && tel.PhysicalBacklog > slaBacklog*0.40 {
			instabilityCount++
		}
		...
		// 2. decision change
		if tick > 1 && dir != timelineLastDecision {
			events = append(events, Event{
				Tick:     tick,
		...
		// 4. latency spike
		if latency > slaLatency || tel.PhysicalBacklog > slaBacklog {
			events = append(events, Event{
				Tick:    tick,
				Type:    "latency_spike",
				Latency: latency,
		...
		// 6. instability high — gate to REAL pressure (backlog > 10% of SLA).
		// Without this gate, ghost instability_high events fire even at zero backlog
		// because BurstEnergy from rising-load creates VarianceScale > 2.0.
		// The event should represent actual system distress, not workload volatility.
		if instScore > 0.5 && tel.PhysicalBacklog > slaBacklog*0.10 {
			events = append(events, Event{
				Tick:    tick,
	...
	recoveryTime := computeRecoveryTime(timeline, maxBacklog)
	advisoryIgnored := 0
	trackErr := 0.0
	convergence := ticks
	retryAmp := 0.0
