...
func (m *MPCOptimiser) mutate(
	...
	bias := ((x.ArrivalMean + x.Backlog*1.0) / (x.ServiceRate + 1e-6))
...
func (m *MPCOptimiser) Optimise(
	...
	// 1) WARM START
	// -------------------------------
	// Incorporate proportional backlog tracking to allow fast gap closure
	warmRequired := (initial.ArrivalMean + initial.Backlog*1.0) / math.Max(initial.ServiceRate, 1)
	for i := 0; i < m.Horizon; i++ {
		if i >= len(prevSeq) {
			seq[i].CapacityTarget = warmRequired
		} else {
			if initial.Backlog > 10 {
				seq[i].CapacityTarget = warmRequired
			} else {
