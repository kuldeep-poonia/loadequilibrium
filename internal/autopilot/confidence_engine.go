...
func ComputeConfidence(prev ConfidenceState, in ConfidenceInput) (float64, ConfidenceState) {
	...
	stabilityFactor := 1.0 / (1.0 + 5.0*shortTermRisk + 3.0*shortTermRisk*shortTermRisk)
...
if instability > 0.8 && (osc > 0.5 || e < 0.15) {
	...
	alpha := 0.18 + 0.25*(1.0-prev.PrevConfidence)
	if alpha > 0.85 {
		alpha = 0.85
	}
	...
	if calmness > 0.7 && in.ControlEffectiveness > 0.4 && conf < 0.6 {
		recovery := 0.04 * (1.0 - conf)
