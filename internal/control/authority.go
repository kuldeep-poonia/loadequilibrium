...
func (a *Authority) deriveBounds(state SystemState, adv AdvisoryBundle, quality *DecisionQuality) ActionBounds {
	...
	if adv.Autopilot.MaxReplicas > 0 {
		// Autopilot cap is a safety ceiling — take the tighter (lower) of policy vs autopilot.
		// Using maxInt here was wrong: it let policy's looser MaxReplicas override the autopilot
		// constraint entirely, meaning the autopilot bound was never actually enforced.
		maxReplicas = minPositiveInt(maxReplicas, adv.Autopilot.MaxReplicas)
	}
